...
def Main(operation, args):
    ...
    if operation == 'init':
        return init()
    if operation == 'addTotalSupply':
        if len(args) != 1:
            return False
        return addTotalSupply(args[0])
    if operation == 'name':
        return name()
    if operation == 'symbol':
        return symbol()
...
def addTotalSupply(amt):
    ...
    """
    assert (CheckWitness(OWNER))
    total = Get(ctx, SUPPLY_KEY)
    Put(ctx, SUPPLY_KEY, total+amt)
    ownerKey = concat(BALANCE_PREFIX, OWNER)
    balance = Get(ctx,ownerKey)
    Put(ctx, ownerKey, balance+amt)
    TransferEvent("", OWNER, amt)
    return True
